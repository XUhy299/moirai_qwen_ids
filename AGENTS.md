...
强候选可显式使用 `--synthetic-sampling epoch_stratified`：每个 epoch 按完整数据包中的五类算子
比例确定性抽取170个互不重叠窗口，5轮最多覆盖850个唯一合成窗，同时保持每轮训练类别比例不变。
协议必须保存逐 epoch 索引哈希、算子计数和跨 epoch 唯一窗口数；旧的 `fixed` 模式仍保留用于
复现实验，运行目录还必须保存精确的 `synthetic_epoch_indices.npy`；不得把两种抽样方式混为同一配置。
...
当前 CLI 已支持机制对照 A（全部80变量数值 Token）、C（54连续 Token + 26离散文本）和
D（全部数值 Token + 额外离散文本）；D 使用显式 `dtt_numeric_mode=all_active`，离散变量按
“语义与原始ID → 数值soft Token → 末端状态文本”对齐。计划中的 B（仅连续数值 Token、无文本）
仍无独立开关；不得用现有参数冒充 B，也不得声称 A/B/C/D 已完整运行。
...
云端32GB RTX 4080 SUPER 的 full Prompt probe 已实测：physical batch=1/2/4 成功，
峰值 reserved 显存分别为7804/13736/25568 MB，batch=6 OOM；按15%显存余量规则正式训练
锁定 physical batch=4。评估 batch 尚无独立容量扫描，先保守锁定为4。不要用 allocator 参数
强行宣称 batch=6 可用。当前代码支持显式 physical/eval batch，不支持梯度累积；在梯度累积
实现并 smoke 前不得在记录中声称使用了梯度累积。
...
- 云端 RTX 4080 SUPER（32230.8 MB）full DTT 容量结果：batch=4 的 peak allocated/reserved
  为23696.1/25568.0 MB，满足15%保留余量并被脚本推荐；batch=6 在约31.37 GiB已用时OOM。
  该结论只锁定工程 batch，不构成 full Prompt 或任何检测方法的效果证据。
