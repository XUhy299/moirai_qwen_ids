...
本文件是仓库顶层 `moirai_qwen_ids/` 后续工作的**主要接手与执行文档**。进入本子项目后，应优先依照本文件和 `PLAN.md` 工作；它不覆盖项目根目录的安全与最终目标约束，`final_goal.md` 始终具有最高研究优先级。如果当前 MOIRAI-Qwen 路线不能带来可靠收益，应主动调整或停止，不得预设 LLM、MOIRAI、Prompt、Reprogramming 或知识蒸馏必然有效。
...
3. 本文件；
4. `README.md`；
5. `PLAN.md`，确认当前未完成项、阻塞项和最近已勾选任务；
6. `record/dtt_isolation_repair/README.md`，掌握当前严格隔离与 compact Prompt 实现；
7. `record/dtt_ablation/README.md` 和 `record/prompt_counterfactual/README.md`，但只把旧 DTT 当失效诊断；
8. `record/moirai_base_layer_scan/README.md`；
9. `record/direct_window_grid/README.md` 和 `record/l32_validation_diagnostic/README.md`；
10. 若涉及基线比较，再读项目根目录 `record/wadi_baseline_comparison.md`；
11. 若涉及旧 MOIRAI Core-set 路线，再读项目根目录 `record/wadi_moirai_small_formal.md` 和 `record/wadi_moirai_realtime_coreset_validation_ablation.md`。
...
正式训练前必须先决定 physical batch、梯度累积和评估 batch，并将实际值写入协议。
